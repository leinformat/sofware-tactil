...
<div class="content-wrapper__table-container">
	...
	<table class="table table-hover table-condensed table-bordered content-wrapper__table-2" id="iddatatable">
		...
		<tbody >
			<?php 
			$result=mysqli_query($conexion,$equipos_entregados);
			while ($datos=mysqli_fetch_row($result)) {
				...
				<tr >
					...
					<td style="text-align: center;">
						<a  href="?mod=cambiar_estatus_st&id-servicio=<?php echo $datos[0];  ?>&nombre_cliente=<?php echo $datos[1]; ?>&fecha_ing=<?php echo $datos[5]; ?>&entregado" class="btn btn-sm icon fa fa-edit fa-lg"> </a>
					</td>

					<td style="text-align: center;">
						<a  target="_blank" href="php/reportesPdf/reporte_equipo.php?imprimir=<?php echo $datos[0] ?>&entregado" class="btn btn-sm icon icon fa fa-print fa-lg"> </a>
					</td>
				</tr>
				<script>
